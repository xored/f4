...
class FanAutoEditStrategy : DefaultIndentLineAutoEditStrategy
{
  ...
  private static const Symbol[] continuationTokens := [ Symbol.backslash,
      Symbol.comma, Symbol.slash, Symbol.plus, Symbol.minus, Symbol.star ]
  ...
  private FanBlock? getLastOpenBlockType(IDocument d, Int start)
  {
    ...
    while (offset > stopPosition)
    {
      ...
      if (lastRegion == null || offset < lastRegion.getOffset) {
        lastRegion = TextUtilities.getPartition(d, partitioning,
            offset, true)
      }
  ...
  private Str getDocumentLine(IDocument d, Int line) { d.get(d.getLineOffset(line), d.getLineLength(line)) }
    ...
    //try {
      ...
      else if (block == null)
      {
        ...
        else
        {
          ...
          if (block != null) {
            peer := scanner.findOpeningPeer(d
                .getLineOffset(lastCodeLine),
  ...
  private Void smartIndentAfterClosingBracket(IDocument d, DocumentCommand c)
  {
    ...
    //try {
      ...
      if (c.offset != d.getLength && bracket == d.get(c.offset, 1)[0]
  ...
  private Bool smartIndentJump(IDocument d, DocumentCommand c)
  {
    ...
    //try {
      ...
      else if (block == null)
      {
        ...
        if (block != null)
        {
          peer := scanner.findOpeningPeer(
              d.getLineOffset(curLine), block.opening,
              block.closing)
          if (peer != null)
            lastCodeLine = d.getLineOfOffset(peer)
        }
  ...
  private Bool previousIsFirstContinuation(IDocument d,
