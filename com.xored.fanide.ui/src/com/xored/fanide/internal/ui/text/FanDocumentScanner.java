...
public class FanDocumentScanner {
	...
	private static class NonWhitespace extends StopCondition {
		/*
		 * @see org.eclipse.dltk.internal.ui.text.FanDocumentScanner.StopCondition#stop(char)
		 */
	...
	private final class NonWhitespaceDefaultPartition extends NonWhitespace {
		...
		/*
		 * @see org.eclipse.dltk.internal.ui.text.FanDocumentScanner.StopCondition#nextPosition(int,
		 *      boolean)
		 */
		public int nextPosition(int position, boolean forward) {
	 ...
	 *            the partition to scan in
	 */
	public FanDocumentScanner(IDocument document, String partitioning,
			String partition) {
	 ...
	 *            the document to scan.
	 */
	public FanDocumentScanner(IDocument document) {
		this(document, IFanPartitions.FAN_PARTITIONING,
	 ...
	 */
	public int findClosingPeer(int start, int depth, final char openingPeer,
		...
		try {
			...
			start -= 1;
			while (backwordstart != NOT_FOUND) {
				backwordstart = scanBackward(backwordstart - 1, UNBOUND, new CharacterMatch(
						new char[] { openingPeer, closingPeer }));
				if (backwordstart == NOT_FOUND)
					continue;

				if (fDocument.getChar(backwordstart) == openingPeer)
					depth++;
				else
					depth--;
			}
	 ...
	 *         <code>bound</code>) that resides in a Fantom partition, or
	 ...
	 *         <code>position</code>] that resides in a Fantom partition, or
	 *         <code>NOT_FOUND</code> if none can be found
	 */
	...
	/**
	 * Checks whether <code>position</code> resides in a default (Fan)
	...
	public IRegion findSurroundingBlock(int offset) {
		...
		int end= findClosingPeer(offset, 0, '{', '}');
